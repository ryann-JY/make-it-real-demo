...
- 社区核心内容对象：Idea、Skill、Substrate、Project、Creator。
- Idea 不是独立供给资产，而是围绕热卖方向、季节、场景或创作主题组织内容的策展专题页；它通过关联 Project、Skill、Substrate 和 Creator 帮助用户理解一个方向可以做成什么。
- Idea 保留独立 URL，但不建设作者、评论、独立投稿、版本管理和后台维护体系；其内容由平台根据 Skill、Substrate、Project、Creator 等资产聚合维护。
- 前端用户可见的基材资产统一命名为 `Substrate / Substrates`，不再出现 `Material` 或 `IDF`；为避免无关架构改动，现有 `/materials` 路由及内部类型、变量、样式类名可继续保留。
- Idea 页面不得做成单件商品详情或纯 Project 瀑布流，也不得在专题层提供统一的成本、售价、利润、制作时间或难度。页面应说明专题范围、典型产品、个性化方式和适用场景，再展示关联 Project、Skill、Substrate 与 Creator。
- Idea 不是可直接执行的工作流，不从专题页直接进入 Studio；用户应先选择具体 Skill 或 Project，再进入 Studio 完成制作。
...
- 首页优先提供四类核心任务入口，再通过 Idea、Skill、Substrate、Project、Creator 的关联内容帮助用户完成判断；五类对象不是必须顺序访问的固定漏斗。
